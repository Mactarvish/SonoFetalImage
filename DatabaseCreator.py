...
dirs = ['/home/hdl2/Desktop/101_ObjectCategories/%d' % i for i in range(101)]

#dirs = [dir_hashimoto_thyroiditis1, dir_hyperthyreosis1, dir_normal1, dir_subacute_thyroiditis1]

def CreateThyDataset(dirs, dataset_name, tv_proportion=5, shuffle=True):
    '''
    ...
    :return: 
    '''
    connection = sqlite3.connect(dataset_name)
    cu = connection.cursor()
    train_set = []
    validation_set = []
    for dir in dirs:
...
CreateImageDatabase("Caltech101")
CreateThyDataset(dirs, "Caltech101", tv_proportion=5, shuffle=True)